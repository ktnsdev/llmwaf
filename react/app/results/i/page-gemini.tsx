...
export default function I() {
  ...
  const handleScroll = () => {
    ...
    const termsContent = document.querySelector("#container .overflow-scroll");
    setIsTermsScrolled(termsContainer.scrollTop === termsContent.scrollHeight);
  };
  ...
  return (
    ...
    >
      ...
      <div className={`mt-4 ${isTermsScrolled ? "" : "opacity-50 pointer-events-none"}`}>
        ...
        <button
          type="button"
          className={`mt-4 py-2 px-4 bg-blue-500 hover:bg-blue-700 text-white font-bold rounded-md ${
            isTermsScrolled ? "" : "opacity-50 pointer-events-none"
          }`}
          onClick={handleSubmit}
        >
          Continue
        </button>
        {!isChecked && isTermsScrolled && (
          <p className="text-red-500 mt-2">You have not accepted the terms of use</p>
        )}
      </div>
    </div>
  );
