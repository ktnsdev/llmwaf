<script>
  import { onMount } from 'svelte';
  import Toast from './ToastGPT.svelte';

  let scrolledToBottom = false;

  // Check if the user has scrolled to the bottom
  function checkScroll() {
    const { scrollTop, clientHeight, scrollHeight } = document.documentElement;
    scrolledToBottom = scrollTop + clientHeight >= scrollHeight;
  }

  // Show toast when the user scrolls to the bottom
  function handleScroll() {
    if (scrolledToBottom) {
      const toastMessage = 'You have reached the bottom of the page!';
      const toast = new Toast({ target: document.body, props: { message: toastMessage } });
    }
  }

  onMount(() => {
    window.addEventListener('scroll', checkScroll);
  });

  // Cleanup event listener
  onDestroy(() => {
    window.removeEventListener('scroll', checkScroll);
  });
</script>

<div id="container" class="container">
  <section>
      <h2>Section 1</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 2</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 3</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <img src="https://source.unsplash.com/random/800x600" alt="Random" />
      <h2>Section 4</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 5</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 6</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 7</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 8</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 9</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 10</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>

      <img src="https://source.unsplash.com/random/800x600" alt="Random" />
  </section>

  <section>
      <h2>Section 11</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 12</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>

      <img src="https://source.unsplash.com/random/800x600" alt="Random" />
  </section>

  <section>
      <h2>Section 13</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>

  <section>
      <h2>Section 14</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>

      <img src="https://source.unsplash.com/random/800x600" alt="Random" />
  </section>

  <section>
      <h2>Section 15</h2>
      <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor voluptas, magni
          architecto laborum suscipit, voluptate doloribus, minus atque impedit nostrum numquam
          maxime dolore! Magni expedita libero natus nobis, distinctio placeat?
      </p>
  </section>
</div>

<style>
  .container {
      padding: 1rem;
      min-width: 100vw;
      min-height: 100vh;
      display: flex;
      flex-direction: column;
      align-items: center;
      gap: 1.25rem;
  }

  h2 {
      font-size: 1.5rem;
      line-height: 2rem;
      font-weight: bold;
  }

  p {
      font-size: 1rem;
      color: #4a5568;
  }

  img {
      width: 100%;
  }
</style>
